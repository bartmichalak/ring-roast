...
- **Open Wearables API**: Integration guide at https://openwearables.io/docs/dev-guides/backend-e2e-integration. Authenticate via `X-Open-Wearables-API-Key` header (NOT Bearer). Credentials from ENV vars: `OPEN_WEARABLES_API_KEY`, `OPEN_WEARABLES_API_URL`. Known doc gaps (provider list endpoint) are in `docs/workshops/ow-integration-reference.md`.
